...
- **`main.bicep`** — declares `Microsoft.MachineLearningServices/workspaces` and binds it to the four pre-provisioned dependent resources.
- **`.github/workflows/deploy-workspace.yml`** — a GitHub Actions pipeline that authenticates via workload identity (no secrets), then runs `az deployment group create` against your resource group.
- **`docs/oidc-federation.md`** — walkthrough for creating the OIDC federated credential on the pre-provisioned UAMI so GitHub can assume it.
...
3. **Configure the OIDC federated credential** on the pre-provisioned UAMI (see `docs/oidc-federation.md`). This lets GitHub Actions exchange its OIDC token for an Azure access token *without any client secret*.
...
5. **Edit `.env.example`** if you want to run `az deployment group create` locally too (copy to `.env`, fill in real values).
6. **Push to `main`** — the workflow fires automatically.
7. **Watch the workflow** in the Actions tab. It logs in via OIDC, then deploys `main.bicep` to your resource group.
...
| File | Purpose |
|------|---------|
| `main.bicep` | Workspace + parameter wiring |
| `.github/workflows/deploy-workspace.yml` | OIDC login + `az deployment group create` |
| `.env.example` | Local dev / manual-deploy variables |
| `docs/oidc-federation.md` | UAMI OIDC federated credential setup |
| `.gitignore` | Bicep / Python / Node ignores |
