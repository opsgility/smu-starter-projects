...
CREATE TABLE IF NOT EXISTS "scenarios" (
	...
	"annual_withdrawal" numeric(18, 2) NOT NULL,
	"expected_return" numeric(6, 4) NOT NULL,
	"return_std_dev" numeric(6, 4) NOT NULL,
	"inflation_rate" numeric(6, 4) NOT NULL,
	"withdrawal_rule" text NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "simulation_results" ADD CONSTRAINT "simulation_results_scenario_id_scenarios_id_fk" FOREIGN KEY ("scenario_id") REFERENCES "public"."scenarios"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
