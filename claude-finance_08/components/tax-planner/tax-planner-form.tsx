...
interface TaxPlannerFormProps {
  defaultNeeded: number;
  defaultAge?: number;
  defaultOtherOrdinaryIncome?: number;
}
...
const FALLBACK_NEEDED = 40_000;
const FALLBACK_AGE = 65;
const FALLBACK_OTHER_INCOME = 30_000;

export function TaxPlannerForm({
  defaultNeeded,
  defaultAge,
  defaultOtherOrdinaryIncome,
}: TaxPlannerFormProps) {
  ...
  const needed = defaultNeeded > 0 ? Math.round(defaultNeeded) : FALLBACK_NEEDED;
  const age = defaultAge ?? FALLBACK_AGE;
  const otherOrdinaryIncome =
    defaultOtherOrdinaryIncome ?? FALLBACK_OTHER_INCOME;
        ...
        <div>
          ...
          >
            ...
            <p className="text-rs-fg-dim text-xs -mt-2">
              Pre-filled from your last projection (or 4% of your current portfolio). Edit any field to plan a different withdrawal.
            </p>
            ...
            <div>
              ...
              <input
                ...
                type="number"
                step="1000"
                defaultValue={otherOrdinaryIncome}
                className="w-full bg-rs-card border border-rs-border rounded-lg px-3 py-2 text-rs-fg font-mono tabular"
              />
              <p className="text-rs-fg-dim text-xs mt-1">
                Social Security, pension, wages — used to stack LTCG and bracket ordinary withdrawals.
              </p>
            </div>
        ...
        <div className="md:col-span-2">
          ...
          {!plan && !error && (
            <div className="bg-rs-surface border border-rs-border rounded-2xl p-12 text-center text-rs-fg-muted">
              Enter your withdrawal needs and run the sequencer to see the recommended pull order: cash → RMD → taxable LTCG → traditional → Roth.
            </div>
          )}
