 ...
 * the payload to extract `sub` and `scope` for the demo. The resource server
 * forwards its validated bearer token here, and the mock mirrors that subject
 * in the responses so students can prove that the downstream service correctly
 * applies access control on a per-user basis.
 *
...
async function main() {
  ...
  app.get("/shipments/:id/history", requireBearer, (req: Request, res: Response) => {
    ...
    // Synthetic top-level history event. The detailed per-event history lives
    // in the local SQLite DB that the resource server reads in get_shipment_history.
