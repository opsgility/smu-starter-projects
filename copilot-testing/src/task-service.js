class TaskService {
  ...
  createTask(title, description, priority = 'medium') {
    ...
    const task = { id: this.nextId++, title: title.trim(), description: description || '', status: 'todo', priority, assignee: null, createdAt: new Date(), updatedAt: new Date() };
  ...
  deleteTask(id) { const idx = this.tasks.findIndex(t => t.id === id); if (idx === -1) throw new Error('Task not found'); this.tasks.splice(idx, 1); return true; }
