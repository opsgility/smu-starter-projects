"""Smoke test for the SIB OSINT Concierge chat API.
...
1. ``POST /chat`` with a prompt that SHOULD trigger ``get_open_source_news``.
...
def main() -> int:
    # 1. Tool-calling turn — expect a `get_open_source_news` call
    print("/chat (tool call expected):")
    result = _chat("What's the latest open-source reporting on balkan-elections?")
    ...
    # 3. Streaming turn
    print("/stream:")
    _stream("Explain RAG in two sentences for an OSINT analyst.")
    print()
