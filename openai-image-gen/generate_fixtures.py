...
- mask.png: 1024x1024 RGBA where a transparent (alpha=0) circle in the
  middle marks the region to be regenerated by gpt-image-1, and the
  opaque (alpha=255) surround marks pixels to preserve.

  OpenAI mask convention (gpt-image-1 / images.edit):
    alpha = 0   -> "edit here"  (regenerate from prompt)
    alpha = 255 -> "keep here"  (preserve original)
...
def make_mask() -> None:
    if os.path.exists("mask.png"):
        ...
        return
    # Start fully opaque white (alpha=255 everywhere => "preserve").
    img = Image.new("RGBA", (SIZE, SIZE), (255, 255, 255, 255))
    draw = ImageDraw.Draw(img)
    r = SIZE // 4
    cx, cy = SIZE // 2, SIZE // 2
    # Punch a fully transparent circle (alpha=0 => "edit this region").
