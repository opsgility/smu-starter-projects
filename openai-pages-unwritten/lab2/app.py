...
Lab 2 starting point: Exercises 1 and 2 are already wired up (storyteller
system prompt + JSON-mode scene generation). In this lab you will add
gpt-image-1 backgrounds (Exercise 3) and structured arc tracking with
ending detection (Exercise 4).
...
# ✓ Completed in Lab 1, then upgraded in Exercise 2 to require JSON output.
...
- Always end on a moment of decision.
...
{{
  ...
  "speaker":   "Narrator",
  "choices":   ["First choice (one short sentence).",
                "Second choice (one short sentence).",
                "Third choice (one short sentence).",
                "Optional fourth choice."]
}}

CHOICE RULES:
- Provide 2 to 4 choices.
- Each choice is one short sentence describing a concrete action the
  protagonist could take next, written in second person ("You ...").
- Choices must follow naturally from the scene you just wrote.
- Choices must be meaningfully different from each other.
- Do NOT include explanations or trailing punctuation other than a period.
